...
\node[dag_partial, dag_template_legend] (legPartial) at (8.4,0) {remaining gap /\\restricted scope};
}
\daglegend{(legDef)(legDone)(legPartial)}{Legend}
...
\node[dag_partial] (T31Lex) at (24.8,0) {
  \textbf{Theorem 3.1 Lexicographic Result} \\
  global value and adjacent-formula optimization across all ties, $M\ge3$
};
...
\node[dag_result] (FiniteRate) at (12.4,-3.3) {
  ...
  finite adjacent-error integrals have the displayed rates
};

\node[dag_result] (T31Branches) at (18.6,-3.3) {
  \textbf{Theorem 3.1 Source Bridge} \\
  nondecreasing matching function supplies positive monotone cell rates
};

\node[dag_partial] (T31) at (24.8,-3.3) {
  \textbf{General-$g$ Theorem 3.1} \\
  identifying the optimized formula with the $W-W_k$ exponent remains open
};
...
\node[dag_partial] (C3) at (6.2,-6.6) {
  \textbf{Lemma C.3} \\
  cross-cell integral rate is proved; its link to $W-W_k$ remains open
};

\node[dag_partial] (C4) at (12.4,-6.6) {
  \textbf{Lemma C.4} \\
  two integral rates are proved; the same-objective equivalence remains open
};
...
\node[dag_result] (C1C2C6C8) at (6.2,-9.9) {
  ...
  convergence, refinement bounds, and general monotone-$g$ C.6
};
...
\node[dag_partial] (T32Cert) at (24.8,-9.9) {
  \textbf{Algorithm 1 / Theorem 3.2} \\
  nested bisection with additive loss and the source runtime rate for $M>3$
};

\node[dag_result] (B1Lemma) at (0,-13.2) {
  \textbf{Lemma B.1} \\
  one-crossing matching-rate shifts order the optimizer coordinates
};
...
\node[dag_partial] (B2B3) at (12.4,-13.2) {
  \textbf{Lemmas B.2-B.3} \\
  finite SLLN learning and rank recovery under strict identifiability
};
...
\node[dag_model] (StateDesign) at (0,-16.5) {
  \textbf{State Dynamics / Section 4} \\
  literal transition, experiment, mixture, and design definitions
};
