...
carrier.  Condition 1 is stated directly for the actual first report and the
actual post-first-report path generated by those interarrivals; the selection
record and its path identifications are then derived definitions.
-/
structure Lemma2ForwardSourceModel
  ...
  startTime : Ω → ℝ≥0
  /-- The fixed paper observation horizon `T` for the selected start. -/
  ...
  start_conditional_law : ∀ᵐ ω ∂P,
    ProbabilityTheory.HasLaw startTime
      (rateFreeStartKernel
        (canonicalFirstArrival (interarrivalPath ω)).toNNReal)
  ...
  start_condIndep_postFirstReportTail :
    ProbabilityTheory.CondIndepFun
      ...
      (((measurable_real_toNNReal.comp
        (measurable_interarrival 0)).comp interarrivalPath_measurable).comap_le)
      startTime (fun ω => futureInterarrival 1 (interarrivalPath ω)) P
...
/-- Condition 1 instantiated with the actual first report and the actual
post-first-report interarrival path of the source Poisson process. -/
def selection (M : Lemma2ForwardSourceModel Ω P) :
    assumption_lemma2_condition_one_selection Ω P (ℕ → ℝ) where
  firstReportTime :=
  ...
  rateFreeStartKernel_isMarkov := M.rateFreeStartKernel_isMarkov
  firstReportTime_measurable := (measurable_real_toNNReal.comp
    (measurable_interarrival 0)).comp M.interarrivalPath_measurable
  startTime_measurable := M.startTime_measurable
  postFirstReportTail_measurable :=
  ...
  start_condIndep_postFirstReportTail :=
    M.start_condIndep_postFirstReportTail

@[simp] theorem selection_firstReportTime
    (M : Lemma2ForwardSourceModel Ω P) :
    M.selection.firstReportTime =
      fun ω => (canonicalFirstArrival (M.interarrivalPath ω)).toNNReal := rfl

theorem firstReportTime_eq
    (M : Lemma2ForwardSourceModel Ω P) :
    M.selection.firstReportTime =
      fun ω => (canonicalFirstArrival (M.interarrivalPath ω)).toNNReal := rfl

@[simp] theorem selection_postFirstReportTail
    (M : Lemma2ForwardSourceModel Ω P) :
    M.selection.postFirstReportTail =
      fun ω => futureInterarrival 1 (M.interarrivalPath ω) := rfl

theorem postFirstReportTail_eq
    (M : Lemma2ForwardSourceModel Ω P) :
    M.selection.postFirstReportTail =
      fun ω => futureInterarrival 1 (M.interarrivalPath ω) := rfl
...
/-- The source-carrier form of Lemma 2 before rewriting the Poisson zero-count
probability as its exponential survival tail. -/
...
theorem windowCount_zero_prob_eq_exponential_tail
    ...
    P.real {ω : Ω |
        S.countProcess.intervalCount W.startTime W.endTime ω = 0} =
      ((AppliedModelingLib.Probability.Exponential.Model.mk S.rate S.rate_pos).measure
...
-- audit-premise: T : AppliedModelingLib.Probability.PoissonProcess.OrderedOneJumpWindow
-- audit-premise: W : ObservationWindow
-- audit-premise: W : AppliedModelingLib.Probability.PoissonProcess.ObservationWindow
-- audit-premise: W : StoppingObservationWindow 𝓕
-- audit-premise: W : AppliedModelingLib.Probability.PoissonProcess.StoppingObservationWindow 𝓕
...
-- audit-premise: Cert : AppliedModelingLib.Probability.PoissonProcess.DurationCensoredFirstCountObservationCertificate 𝓕
-- audit-premise: caseFromWindow : ObservationWindow → Theorem2ObservedWindowCase
-- audit-premise: caseFromWindow : AppliedModelingLib.Probability.PoissonProcess.ObservationWindow → LBG24SpatialUnderreporting.Theorem2ObservedWindowCase
-- audit-premise: anonymous : FirstCountArrivalCertificate 𝓕 reportCount startTime
-- audit-premise: anonymous : AppliedModelingLib.Probability.PoissonProcess.FirstCountArrivalCertificate 𝓕 reportCount startTime
-- audit-premise: anonymous : FirstCountArrivalCertificate 𝓕 reportCount firstReportTime
-- audit-premise: anonymous : AppliedModelingLib.Probability.PoissonProcess.FirstCountArrivalCertificate 𝓕 reportCount firstReportTime
