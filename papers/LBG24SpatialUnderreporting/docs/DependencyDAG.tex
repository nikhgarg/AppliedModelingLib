...
\node[dag_unformalized] (ShiftedProcess) at (11.5,-8) {
  \textbf{Appendix shifted-process claim (uncredited)} \\
  after the first jump, the remaining gaps are \\
  iid exponential at the original rate
};

\node[dag_unformalized] (Lemma2) at (17.5,-8) {
  \textbf{Lemma 2 (uncredited)} \\
...
\draw[dag_dashed_arrow] (ReportingModel.east) -- ++(1.0,0) |- ([yshift=0.7cm]ShiftedProcess.north) -- (ShiftedProcess.north);
\draw[dag_dashed_arrow] (StartCondition.south) to[out=-90,in=90] (Lemma2.north);
\draw[dag_dashed_arrow] (ShiftedProcess) -- (Lemma2);
\draw[dag_dashed_arrow] (Lemma2.north east) -- (Theorem.south west);
